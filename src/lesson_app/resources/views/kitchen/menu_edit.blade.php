...
@section('head_title','メニュー管理：編集')
@section('main_title','メニュー管理：編集')
@section('main')
            ...
            <div class="menu_list">
                  <table class="table">
                        ...
                        <!-- @foreach($menus as $menu)
                              <tr>
                              <td><input class="form-check-input" type="radio" name="menu_id" value="{{ $menu->id }}" id="{{ $menu->id }}"></td>
                              <td><label class="w-100 h-100" for="{{ $menu->id }}">{{$menu->menu_no}}</label></td>
                              <td><label class="w-100 h-100" for="{{ $menu->id }}">{{$menu->category->category_name}}</label></td>
                              <td><label class="w-100 h-100" for="{{ $menu->id }}">{{$menu->menu_name}}</label></td>
                              <td><img src=""></td>
                              <td><label class="w-100 h-100" for="{{ $menu->id }}">{{$menu->price}}</label></td>
                              </tr>
                        @endforeach -->
                        </tbody>
                  </table>
